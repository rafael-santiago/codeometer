...
func TestPantheonEstimatorK(t *testing.T) {
    p := PantheonEstimator{}
    if p.K() != 14 {
        t.Error(`p.K() != 14`)
    }
...
func TestPantheonEstimatorEstimate(t *testing.T) {
    testVector := []struct {
                    BytesTotal int64
                    ExpectedMessage string
                  }{
                    { 10, `Your code has 0.26% of the Pantheon's height (14m).` },
                    { 512 << 6, `Pantheon's height (14m) has 11.80% of your code extension (118.68m).` },
                 }
